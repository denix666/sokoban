        ...
        match game_state {
            ...
            GameState::Game => {
                ...
                if is_key_pressed(KeyCode::Left) {
                    let check_x: i32 = ((player.x - 64.0) / 64.0) as i32;
                    let check_y: i32 = (player.y / 64.0) as i32;
                    
                    let mut can_move_package: bool = true;

                    if crate::map::get_val(check_x, check_y, &points) != "#" {
                        for package in &mut packages {
                            if check_x - 1 == (package.x / 64.0) as i32 && check_y == (package.y / 64.0) as i32 {
                                can_move_package = false;
                            }
                        }
                        for package in &mut packages {
                            if check_x == (package.x / 64.0) as i32 && check_y == (package.y / 64.0) as i32 {
                                let check_px: i32 = ((package.x - 64.0) / 64.0) as i32;
                ...
                if is_key_pressed(KeyCode::Up) {
                    let check_x: i32 = (player.x / 64.0) as i32;
                    let check_y: i32 = ((player.y - 64.0) / 64.0) as i32;

                    let mut can_move_package: bool = true;
                    
                    if crate::map::get_val(check_x, check_y, &points) != "#" {
                        for package in &mut packages {
                            if check_x == (package.x / 64.0) as i32 && check_y - 1 == (package.y / 64.0) as i32 {
                                can_move_package = false;
                            }
                        }
                        for package in &mut packages {
                            if check_x == (package.x / 64.0) as i32 && check_y == (package.y / 64.0) as i32 {
                                ...
                                let check_py: i32 = ((package.y - 64.0) / 64.0) as i32;
            ...
            GameState::InitLevel => {
                ...
                for point in &mut points {
                    match point.value.as_str() {
                        "@" => {
                            player.x = point.x as f32 * 64.0;
                            player.y = point.y as f32 * 64.0;
                            break;
                        },
                        _ => {
                        },
                ...
                for point in &mut points {
                    match point.value.as_str() {
                        "$" => {
                            packages.push(
                                Package::new(point.x as f32 * 64.0, point.y as f32 * 64.0).await
                            );
                        },
                        "*" => {
                            packages.push(
